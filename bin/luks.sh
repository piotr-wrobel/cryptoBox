#!/bin/bash
    ...
    function nameVol ()
    {
        ...
        if [[ ! -n "$vol_name" ]]; then
            vol_name='EncryptedContainer.cnt';
            mount_dir='EncryptedContainer'
        else
          mount_dir="$vol_name"
          vol_name="${vol_name}.cnt"
        fi
    };
    function nameKey ()
    {
    ...
    function encryptCon ()
    {
        ...
        sudo cryptsetup -y -c aes-xts-plain64 -s 512 -h sha512 -i 5000 --use-random luksFormat "$vol_name" && notification "Encrypted container created."
    };
    function encryptOpen ()
    {
        #sudo cryptsetup luksOpen "$vol_name" "$mount_dir" --key-file "$key_file" && notification "Volume unlocked."
        sudo cryptsetup luksOpen "$vol_name" "$mount_dir" && notification "Volume unlocked."

    };
    function encryptClose ()
    {
      sudo dmsetup remove /dev/mapper/"$mount_dir"
    };
    function mkfsFormat ()
    {
